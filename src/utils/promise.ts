...
export function promiseAllSettledLogErrors<T>(results: PromiseSettledResult<T>[]){
  const rejectedResults = results.filter(isRejected)
  if(rejectedResults && rejectedResults.length > 0){
    rejectedResults.forEach((item: PromiseRejectedResult) => console.error(item.reason))
  }
}